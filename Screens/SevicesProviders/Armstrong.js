...
const Armstrong = () => {
  ...
  const toggleLocation = async (value) => {
    ...
    await AsyncStorage.setItem("ArmisLocationEnabled", JSON.stringify(value)); // Persist state
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (deleteResponse.status === 200) {
        ...
        await AsyncStorage.removeItem("ArmisLocationEnabled");
      } else {
        Alert.alert("Error", "Something went wrong. Please try again.");
      }
    } catch (error) {
